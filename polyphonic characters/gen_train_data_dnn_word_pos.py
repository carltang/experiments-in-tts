...
def loadw2v(datapath):
    global W2V
    w2vpath = datapath + "/dim100.w2v.bin"
...
def run_frontend(input, output):
    os.system("./../frontend_xcode ./../data {} > {}".format(input, output))
...
def trans_to_dnn_train(hanzi, seg, label, outputDir):
    ...
    while True:
        ...
        for i in keyindex:
            X = np.zeros((11, W2V_LEN + POS_LEN), dtype=np.float32)
            for j in range(0, 11):
                if i + j - 5 >= 0 and i + j - 5 < len(cursentence):
                    try:
                        w_id = W2V[cursentence[i+j-5]["word"]]
                    except:
                        print("can not find {} in w2v".format(cursentence[i]["word"]))
                        w_id = pad_arr_word
                    try:
                        pos_id = P2V[cursentence[i+j-5]["pos"]]
                    except:
                        pos_id = pad_arr_pos
                    X[j, :W2V_LEN] = w_id
                    X[j, W2V_LEN:] = pos_id
...
def genData():
    ...
    os.system("mkdir dnn_train_data_word_pos")
    if not os.path.exists("word_train_middle"):
        os.mkdir("word_train_middle")

    # 先转换training data
    trainInputList = GetFileList("combin_data/without_label", [])
    ...
    #for line in lines:
        ...
        labelFile = "combin_data/with_label/" + hanzi + "_test.txt"
