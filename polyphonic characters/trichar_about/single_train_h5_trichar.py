...
'''
def cnn_2d_2_dense_model(params, num_classes):
	# create model
	model = Sequential()
	model.add(Conv2D(32, (2, 32), input_shape=(11, 143, 1), strides=(1, 5), activation='relu'))
	model.add(MaxPooling2D(pool_size=(2, 2)))
	model.add(Conv2D(32, (3, 5), activation='relu'))
	model.add(MaxPooling2D(pool_size=(2, 2)))
	#model.add(Dropout(params[8]))
	model.add(Flatten())
...
def try_params(hanzi, params):
	if not os.path.exists("model_trichar"):
		os.system("mkdir model_trichar")
	resultFile = codecs.open("model_trichar/evaluate_{}_{}.txt".format(params, hanzi), 'w', 'gb18030')
...
if __name__=='__main__':
	...
	fp = codecs.open("train_results_trichar.txt", "a", "gb18030")
